...
const BuildOMeter = ({ currentAmount, goal, onUpdateGoal }) => {
    ...
    const percentage = Math.min(100, Math.max(0, (currentAmount / goal.itemPrice) * 100));
    const totalStuds = 12;
    const filledStuds = Math.floor((percentage / 100) * totalStuds);
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-start">
                ...
                <div className="flex-1 space-y-4">
                    <div className="flex items-start justify-between gap-2">
                        <h3 className="text-xl font-bold text-lego-black leading-tight">{goal.itemName}</h3>
                    ...
                    {/* Stud progress row */}
                    <div>
                        <p className="text-xs font-bold text-lego-black/50 uppercase tracking-widest mb-2">Build Progress</p>
                    ...
                    {percentage >= 100 && (
                        <div className="flex items-center justify-center gap-2 text-lego-green font-bold animate-bounce">
                            <Trophy size={18} />
                            <span>Goal Reached! Time to build!</span>
                            <Trophy size={18} />
                        </div>
                    )}
                </div>
